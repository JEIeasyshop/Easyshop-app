...
import { Plus, Package, Download, Search, Pencil } from 'lucide-react'
...
export default function OrdersTab({ orders, tracking, addOrder, updateOrder, customers = [], addCustomer }) {
  ...
  return (
    <div>
                ...
                {filtered.map(o => {
                  ...
                  return (
                    <tr key={o.id}>
                      ...
                      <td>
                        <button className="btn-ghost btn-sm" title="Edit order"
      ...
      {/* Edit order form — pre-fills from existing order */}
      {editOrder && (
        <OrderForm
...
export default function OrdersTab({ orders, tracking, addOrder, customers = [], addCustomer }) {
  const [showForm, setShowForm] = useState(false)
  const [search, setSearch]     = useState('')
